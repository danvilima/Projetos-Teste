        ...
        for j, col in enumerate(cols):
            if i + j < len(groups_list):
                ...
                with col:
                    ...
                    pos1 = st.selectbox("1º Colocado", opts1, index=idx1, key=f"sel_{g}_1")
                    if pos1 != s1:
                    ...
                    pos2 = st.selectbox("2º Colocado", opts2, index=idx2, key=f"sel_{g}_2")
                    if pos2 != s2:
                    ...
                    pos3 = st.selectbox("3º Colocado", opts3, index=idx3, key=f"sel_{g}_3")
                    if pos3 != s3:
                        st.session_state[f"group_{g}_3"] = pos3
                        st.rerun()
                    
                    st.markdown("<div class='random-btn'>", unsafe_allow_html=True)
                    if st.button("🎲 Sorteio Aleatório", key=f"btn_rand_{g}"):
                        randomize_group(g)
                        st.rerun()
