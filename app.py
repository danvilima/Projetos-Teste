...
# Header
st.markdown(
    "<h1 class='accent-text'>Simulador da Copa do Mundo 2026</h1>",
    unsafe_allow_html=True,
)
st.write(
    "Quem levantará a taça? Escolha os dois primeiros colocados de cada grupo e oito terceiros colocados para formar os 32 que vão para o mata-mata. Depois, monte a chave até a final."
)
...
if is_unlocked or st.session_state.get("third_place_matching"):
    ...
    if champion:
        ...
        user_name = st.text_input(
            "Seu nome para salvar o placar (opcional):", "Anonymous"
        )

        if st.button("💾 Salvar Simulação no Supabase"):
            if save_simulation(
                champion, runner_up or "N/A", third_place or "N/A", user_name
            ):
                st.balloons()
